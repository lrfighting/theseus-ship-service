  ...
  return [
    { role: 'system', content: header(template, SCHEMA_SUMMARY) },
    {
      ...
      content: [
        ...
        `## story_summary 的写法要求`,
        `除描述故事主要内容外，请特别说明：`,
        `  1. 故事的核心悲剧或遗憾是什么（如果存在）`,
        `  2. 哪一个时刻是整个故事命运的真正起点（读者读完后会觉得"从那一刻开始，一切就注定了"）`,
        `  3. 读者最想改变的那个瞬间是什么（"如果当时……会不会不一样"）`,
        ``,
        `## themes 的写法要求`,
        `除故事类型标签外，请包含推演视角的主题标签，如：`,
        `  "命运抉择" / "蝴蝶效应" / "不可逆的一步" / "沉默的代价" / "错过的机会" / "未说出口的话"`,
        ``,
        storyBlock(input),
      ].join('\n'),
    },
  ...
  return [
    { role: 'system', content: header(template, SCHEMA_WORLD) },
    {
      ...
      content: [
        ...
        ``,
        `### 第一层：表层世界（已发生的）`,
        `- scenes：描述故事中出现的实际场景（至少 1 个）`,
        `- 角色所处的社会/权力结构（谁拥有权力，谁受到约束）`,
        ``,
        `### 第二层：深层规则（核心，用于推演平行命运）`,
        `rules 字段必须是这个故事世界中的"因果法则"，格式为 [条件] → [后果]。`,
        `至少提炼 3 条，覆盖权力、人际关系、信息等不同维度。`,
        ``,
        `✅ 正确的 rules 示例（因果法则）：`,
        `  - "在这个朝廷，抗旨不遵者无论功劳多大必遭清算"`,
        `  - "在这个家庭，女儿的付出被视为理所当然，拒绝则意味着不孝"`,
        `  - "一旦秘密被第三人知晓，当事双方关系将无法回到原来的状态"`,
        `  - "承认脆弱的人，在这个环境中会被他人利用或轻视"`,
        ``,
        `❌ 禁止出现的 rules（剧情事件，不是规则）：`,
        `  - "主角被人陷害" ← 这是事件`,
        `  - "某人在火灾中去世" ← 这是事件`,
        `  - "男主注册了微博" ← 这是事件`,
        ``,
        `## core_conflict 的写法`,
        `说明这个故事世界里最根本的结构性张力，不是"A和B有矛盾"，而是：`,
        `  - "个人情感 vs 家族/权力体制的碾压"`,
        `  - "真相 vs 利益方维持谎言的强烈动机"`,
        `  - "自我意志 vs 命运/环境的不可抗力"`,
        ``,
  ...
  return [
    { role: 'system', content: header(template, SCHEMA_CHARACTERS) },
    {
      ...
      content: [
        ...
        `## 角色分析深度要求（用于平行命运推演）`,
        ``,
        `**motivation（动机）区分两层，格式："表层：xxx / 深层：xxx"**`,
        `  - 表层动机：角色公开表现出来的目标或行为理由`,
        `  - 深层动机：角色真正渴望的东西（往往与表层不同，甚至相互矛盾）`,
        `  - 例："表层：尽职守护家人 / 深层：渴望被认可、摆脱"工具人"角色"`,
        ``,
        `**personality（性格）包含三项：**`,
        `  1. 核心特质（1-2个关键词，如"隐忍/爆发型"）`,
        `  2. 在压力下的反应模式（性格底色）`,
        `  3. 崩溃临界点：什么情况下会做出出乎意料的选择（这是分支续写的关键）`,
        ``,
        `**background（背景）重点说明：**`,
        `  - 什么过往经历塑造了他现在的选择模式`,
        `  - 有哪些不为人知的经历影响其行为`,
  ...
  return [
    { role: 'system', content: header(template, SCHEMA_RELATIONS) },
    {
      ...
      content: [
        ...
        '## 关系分析深度要求（用于平行命运推演）',
        '',
        '**relation 字段包含两层，格式："表层：xxx / 深层张力：xxx"**',
        '  - 表层：当前呈现给外界的关系标签（如"主仆""兄妹""对手"）',
        '  - 深层张力：关系背后的隐性矛盾或未解情感',
        '  - 例："表层：忠诚同盟 / 深层张力：A知道一个会毁掉B的秘密，选择沉默"',
        '',
        '**description 字段重点说明：**',
        '  1. 这段关系中最脆弱的点是什么（什么事件会让它崩塌）',
        '  2. 什么会让这段关系发生根本性转变',
        '  3. 有没有"一旦说出口就会改变一切"的秘密或情感',
        '',
        '**intensity（强度）代表情感卷入程度和对主线命运的影响权重：**',
        '  - high：深度卷入，这段关系的变化对双方或故事走向影响巨大',
        '  - mid：中度关联，变化影响主线发展',
        '  - low：表层接触，变化对主线影响有限',
        '',
        `### 人物列表`,
  ...
  return [
    { role: 'system', content: header(template, SCHEMA_KEY_NODES) },
    {
      ...
      content: [
        ...
        ``,
        `关键节点 ≠ 普通叙事分叉`,
        `关键节点 = 读者读完整个故事，回头看时会心想："就是这一刻，如果当时选了另一条路，整个命运都会不同"`,
        ``,
        `判断一个段落是否是命运拐点，问自己三个问题：`,
        `  1. 如果这里发生了不同的选择，后续的命运走向（生死/关系/结局）会有实质性改变吗？`,
        `  2. 有没有一种"无法回头"的感觉——这个选择一旦做了，就再也回不去了？`,
        `  3. 读者会不会在这里心跳加速，因为他知道"接下来的选择会改变一切"？`,
        ``,
        `满足以上任意两条，才是命运拐点。`,
        ``,
        `## 高质量命运拐点的典型场景（优先标记）`,
        ...
        ``,
        `★ 背叛/选择边界：`,
        `  - "他知道，此刻的选择意味着和某一方永远决裂"`,
        `  - "只要沉默，一切还来得及；只要开口，就再也无法装作不知道"`,
        ``,
        `★ 蝴蝶效应起点（最高优先级）：`,
        `  - 表面上微小、实际上牵一发而动全身的选择`,
        `  - 读者事后会感慨"要是当时没有这个选择，后面的一切悲剧就不会发生"`,
        ``,
        `## ❌ 绝对不能标为节点的段落类型（反例）`,
        `以下段落即使出现在文中，也绝不能输出为 key_nodes：`,
        `  - 纯时间/空间推进："第二天一早"、"三个月后"——只是过渡，无抉择；`,
        `  - 纯信息交代：角色背景介绍、世界观铺陈、日常琐事描述；`,
        ...
        `[2] 再识别：检查段落是否属于"命运拐点典型场景"（优先）或包含"正例"中的抉择张力。`,
        `[3] 后评分：按"命运影响程度 × 不可逆程度 × 情绪强度"打分。`,
        `[4] 最后做过滤：去重、间隔控制、篇幅分段数量控制。`,
        ``,
        `## 硬性门槛`,
        `只有同时满足下列条件的段落，才允许作为 key_nodes 的一项：`,
        `  - 该段能让读者产生"如果选了另一条路，后续命运会实质性不同"的感受；`,
        `  - 段落内出现叙事岔路信号之一：转折词/决策动词/心理权衡/直接疑问/对峙或秘密揭露等；`,
        `  - 禁止把「纯时间推进、纯场面描写、纯信息交代」这类无命运张力的句子标为节点。`,
        ``,
        `## 筛选规则（必须遵守）`,
        `1. 去重与密度控制：`,
        ...
        ``,
        `3. 重要性分级：`,
        `   - importance = "main"：重大命运转折（改变主线走向、核心人物生死、重大真相揭露）`,
        `   - importance = "side"：次级抉择（丰富人物关系、埋下伏笔、支线选择）`,
        ...
        ``,
        `❌ 低质量选项（应对策略型，禁止出现）：`,
        `  - "先冷静思考" / "观察形势" / "等待时机" ← 都是"如何更好地处理同一件事"，无命运分叉`,
        `  - "向某人求助" / "暗中调查" / "静观其变" ← 只是策略，不是命运选择`,
        `  - "继续观察局势，等待更好时机" ← 泛化废话`,
        ``,
        `✅ 高质量选项（平行命运型）：`,
        `  每个选项对应一种完全不同的命运走向，让读者感到：`,
        `  "如果我选了这个，接下来的世界会完全不同"`,
        ``,
        `  - 选项A（冲突升级/主动改写）：打破现状，承担最大风险，可能改写命运`,
        `  - 选项B（稳健承受/顺从命运）：咽下这一刻，沉默接受，看命运将走向何处`,
        `  - 选项C（意外反转/第三条路）：拒绝非此即彼，用出乎意料的方式打破僵局`,
        ``,
        `### 选项正例（好选项）`,
        `原文节点：她看着那碗药，心知喝下去就再也回不去了。`,
        `  1. 饮下毒药——将药一饮而尽，承担未知后果，用生命换取对方信任`,
        `  2. 摔碗拒绝——打翻药碗，当场揭穿对方意图，正面决裂`,
        ...
        ``,
        `### 选项规则（必须遵守）`,
        `1. 标题在前：破折号前是5-12字的选择标题，必须体现具体行动或态度，不能是"继续观察""随机应变"。`,
        `2. 描述在后：破折号后是20-40字的具体描述，补充行动后果或动机。`,
        `3. 严禁重复原文：不要在选项中重复原文的大段内容，不要出现"仍处在/语境/仍基于"等废话。`,
        `4. 差异性：三个选项必须是三种不同价值观/风险偏好的真实命运走向，不能是"好/更好/最好"。`,
        `5. 可行性：符合该角色性格与世界规则，不能出现角色做不出的选择。`,
        `6. 悬念感：每个选项的后续结果不明确，激发读者好奇"选了之后会发生什么"。`,
        `7. 禁止脱锚：严禁引入该段落未出现的人名、机构、网络舆论、热搜、公关、实锤、风水玄学等。`,
        `8. tone 标注：每个选项必须标注 tone（走向风格），如：稳健、冲突升级、关系扩展、意外反转。`,
        ``,
        `## 输出要求`,
        ...
        `  - confidence 表示你对这个节点判断的置信度（0.0-1.0）。只有真正确信是命运拐点的才给高置信度。`,
